<template>
    <div class="flex flex-col mt-20 mx-4">
        <h1 class="text-center text-4xl font-bold my-4 mb-12 text-blue-600">
            Vue TODO
        </h1>
        ...
        <div
            :class="
                todos.length > 0 || completed.length > 0
                    ? 'w-1/3 mx-auto p-5 mt-5 bg-gray-200 rounded-md'
                    : 'w-1/3 mx-auto p-5 mt-5'
            "
        >
            <div v-if="isAll">
                <ul id="todo-list">
            ...
            >
                <span v-if="todos.length > 0"
                    >{{ todos.length }}
                    {{ todos.length > 1 ? "items" : "item" }} left</span
                >
                <div>
                    <button
                        type="button"
                        class="px-2 mr-1 rounded-md"
                        @click="isAll = true"
                    >
                        All
                    </button>
                ...
                <span class="cursor-pointer" @click="deleteCompleted">{{
                    completed.length > 0 ? "Clear completed" : ""
                }}</span>
            </div>
        </div>
    </div>
